...
- MODULE: app.core.desktop.components.standalone.diffusion_widget
- RESPONSIBILITY: Standalone UI for Diffusion plugin.
- CALLED BY: MainWindow via direct script execution.
- CALLS TO: app.core.shared_registry (DetectorFactory, DiffusionMainModelFactory)
- IN = OUT: Runs detection then diffusion inpainting on an image.
=============================================================================
"""
import os
import threading
...
class DiffusionStandaloneWidget(QWidget):
    ...
    def _populate_models(self):
        ...
        diff_keys = config_loader.list_field_keys("diffusion_main_model")
        for key in diff_keys:
            if key != "none":
                self.combo_diffusion.addItem(config_loader.format_display_label(key, "diffusion_main_model"), key)

    def _on_load_model(self):
        det_key = self.combo_detector.currentData()
        diff_key = self.combo_diffusion.currentData()
        
        if not det_key or not diff_key:
            return
        ...
        def _load():
            try:
                config_dict = {
                    ...
                    "inpainter": {"diffusion_model": diff_key, "enable_advanced_diffusion": True},
    ...
    def _on_select_image(self):
        ...
        if path:
            ...
            if self.detector_instance and self.diffusion_instance:
                self.btn_run.setEnabled(True)

    def _on_run(self):
        if not self.current_image_path or not self.detector_instance or not self.diffusion_instance:
            return
        ...
        def _process():
            try:
                ...
                res_img = self.diffusion_instance.inpaint(img, bboxes)
    ...
    def _on_process_success(self, res_img: np.ndarray):
        self.btn_run.setEnabled(True)
        self.btn_run.setText("Run Diffusion")
...
if __name__ == "__main__":
    ...
    os.environ["TARGET_PLUGIN_DIRS"] = "detector,diffusion"
    
    from PySide6.QtWidgets import QApplication
    app = QApplication(sys.argv)
    app.setStyle("Fusion")
    window = DiffusionStandaloneWidget()
