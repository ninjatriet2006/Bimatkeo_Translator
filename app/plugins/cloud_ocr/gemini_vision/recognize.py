...
- MODULE: app.plugins.cloud_ocr.gemini_vision.recognize
- RESPONSIBILITY: Thực thi nhận diện chữ (Cloud OCR) qua Gemini Vision API.
...
def recognize_gemini_vision(ocr_instance, image: np.ndarray, lang: str = "en") -> list[dict]:
    if not ocr_instance.api_key:
        if ocr_instance.log_callback:
            ocr_instance.log_callback("ERROR", "API Key cho Gemini Vision không được cung cấp.")
        return []
    ...
    if ocr_instance.endpoint:
        base_url = ocr_instance.endpoint.format(model=ocr_instance.model_name) if "{model}" in ocr_instance.endpoint else ocr_instance.endpoint
        url = f"{base_url}?key={ocr_instance.api_key}" if "?" not in base_url else f"{base_url}&key={ocr_instance.api_key}"
    else:
    ...
    try:
        ...
        if ocr_instance.log_callback:
            ocr_instance.log_callback("INFO", f"Gemini Vision phát hiện {len(valid_results)} khối chữ.")
        return valid_results

    except urllib.error.URLError as e:
        if ocr_instance.log_callback:
            ocr_instance.log_callback("ERROR", f"Lỗi kết nối Gemini API: {e}")
        return []
    except json.JSONDecodeError as e:
        if ocr_instance.log_callback:
            ocr_instance.log_callback("ERROR", f"Không thể parse JSON từ Gemini: {e}")
        return []
    except Exception as e:
        if ocr_instance.log_callback:
            ocr_instance.log_callback("ERROR", f"Lỗi không xác định khi gọi Gemini OCR: {e}")
        return []
